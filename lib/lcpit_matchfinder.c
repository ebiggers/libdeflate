		       ...
		       u32 min_match_len, u32 nice_match_len)
{
	...
	mf->pos_data = libdeflate_malloc(get_pos_data_size(max_bufsize));
	mf->intervals = libdeflate_malloc(get_intervals_size(max_bufsize));
...
{
	libdeflate_free(mf->pos_data);
	libdeflate_free(mf->intervals);
}
